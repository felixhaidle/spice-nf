...
process FAS_SCORE_CALCULATION {
    ...
    if [ -f "tmp/${gene_id}.tsv" ]; then
        ...
        # Run FASResultHandler to concatenate results
        echo "Starting: Concatenation for gene ${gene_id}"
        python ${projectDir}/tools/SPICE/FASResultHandler.py \
            --mode concat \
            --gene_id "${gene_id}" \
            --out_dir "tmp/fas_${gene_id}" \
            --anno_dir "${spice_library}/fas_data/"
        echo "Finished: Concatenation for gene ${gene_id}"

        # Run FASResultHandler to delete temporary data
        #echo "Starting: Cleanup for gene ${gene_id}"
        #python ${projectDir}/tools/SPICE/FASResultHandler.py \
        #    --mode delete \
        #    --gene_id "${gene_id}" \
        #    --out_dir "tmp/fas_${gene_id}"
        #   echo "Finished: Cleanup for gene ${gene_id}"
...
process PARSE_DOMAIN_OUTPUT {
    ...
    publishDir "${outdir}/${spice_library}", mode: 'copy'
